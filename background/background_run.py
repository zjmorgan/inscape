...
def background(runs, proc, all_runs, outname, outdir, dbgdir, tube_calibration, detector_calibration, mask_file, instrument, ipts, banks, det_size, N_ws_bkg, cols, rows, k_min, k_max, k_step):
    ...
    for r in runs:
        ...
        ws_bkg = GenerateGoniometerIndependentBackground(InputWorkspaces='ws_ev',
                                                         GroupingFile=os.path.join(dbgdir, '{}_group_{}x{}.xml'.format(instrument,rows,cols)),
        ...
        SaveNexus(InputWorkspace='ws_bkg',
                  Filename=os.path.join(outdir, '{}_{}_bkg.nxs'.format(instrument,r)))
...
if __name__ == '__main__':
    ...
    while spectrumInfo.isMonitor(i):
        i += 1
    CropWorkspace(instrument, StartWorkspaceIndex=i, OutputWorkspace=instrument)
    ...
    with open(os.path.join(dbgdir,'{}_group_{}x{}.xml'.format(instrument,rows,cols)),'wt+') as f:
    ...
    args = [run_nos, outname, outdir, dbgdir, tube_calibration, detector_calibration, mask_file, instrument, ipts, banks, det_size, N_ws_bkg, cols, rows, k_min, k_max, k_step]
    ...
    os.remove(os.path.join(dbgdir, '{}_group_{}x{}.xml'.format(instrument,rows,cols)))